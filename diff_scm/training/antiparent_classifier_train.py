...
def main(args):
    ...
    for step in range(int(config.classifier.training.iterations) - resume_step):
...
def save_model(mp_trainer, opt, step):
    if dist.get_rank() == 0:
        th.save(
            ...
            os.path.join(logger.get_dir(), "best_model.pt"),
        )
        # th.save(opt.state_dict(), os.path.join(logger.get_dir(), f"opt{step:06d}.pt"))
        th.save(opt.state_dict(), os.path.join(logger.get_dir(), f"best_opt.pt"))
