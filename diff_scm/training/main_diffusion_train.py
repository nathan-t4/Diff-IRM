...
from diff_scm.utils.script_util import create_gaussian_diffusion, create_score_model, create_noise_encoder
from diff_scm.training.train_util import TrainLoop, MultiTrainLoop
...
def main(args):
    ...
    logger.configure(Path(os.path.join(config.experiment_path, config.experiment_name, "score_train", "test_2")),
                     format_strs=["log", "stdout", "csv", "tensorboard"])

    logger.log("creating model and diffusion...")
    diffusion = create_gaussian_diffusion(config)
    model = create_score_model(config)
    noise_encoder = create_noise_encoder(config)
